...
export const getNotes = async (accessToken: string) => {
    ...
    return axios.get(`${localHostURL}/api/v1/note/getNotes`, {headers: { ContentType: 'application/json', Authorization: `Bearer ${accessToken}` }, withCredentials: true})
}
...
export const addNote = async (title: string, content: string, preview: string, accessToken: string) => {
    // server has the access to the cookies in case access token is invalid
    return axios.post(`${localHostURL}/api/v1/note/addNote`, {title, content, preview} ,{headers: { ContentType: 'application/json', Authorization: `Bearer ${accessToken}` }, withCredentials: true});
};
...
export const updateNote = async (title: string, content: string, preview: string, accessToken: string, noteId: string) => {
    // server has the access to the cookies in case access token is invalid
    return axios.put(`${localHostURL}/api/v1/note/editNote/${noteId}`, {title, content, preview}, {headers: { ContentType: 'application/json', Authorization: `Bearer ${accessToken}` }, withCredentials: true});
};
...
export const deleteNote = async (accessToken: string, noteId: string) => {
    // server has the access to the cookies in case access token is invalid
    return axios.delete(`${localHostURL}/api/v1/note/deleteNote/${noteId}`, {headers: { ContentType: 'application/json', Authorization: `Bearer ${accessToken}` }, withCredentials: true});
};